...
*/15 * * * * clamav venv/bin/fangfrisch -c /etc/fangfrisch.conf -s /dev/log refresh
----

== Logging

Fangfrisch uses console logging via stdout/stderr streams as a default.
You can use the `--syslog TARGET` option to log to https://tools.ietf.org/html/rfc5424[syslog] instead.
Possible targets are absolute file paths, hostname, and hostname:udpport, as shown in the following examples:

* `/dev/log` - Unix domain socket on local machine.
* `localhost` - Local syslog, listening on default UDP port 514.
* `host.example.com:4321` - Remote syslog, listening on custom UDP port 4321.

Two environment variables can be used to influence what messages are being logged:

* LOG_LEVEL: Select one of DEBUG, INFO, WARNING, ERROR or FATAL. Default: WARNING.
* LOG_FORMAT: See https://docs.python.org/3/library/logging.html#logging.Formatter[Formatter class] documentation for details.
Default: Sensible choices depending on the selected log target.
...
1. Have you checked the documentation?
2. Have you checked all existing issues, including closed ones?
3. Is the feature I am suggesting helpful for more people than just myself?

If you answered "yes" for all questions, please explain your idea as detailed as possible.
