...
An attempt will be made to drop all existing tables.
If that is not possible, you need to fix the underlying issue manually, depending on your database type.
For SQLite, deleting the database file is usually the easiest method.
