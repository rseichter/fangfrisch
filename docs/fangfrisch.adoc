...
The default is the current working directory of the Python process, which can vary depending on how you launch Fangfrisch.
It is highly recommended to define an absolute path like `/var/lib/clamav` instead.
* *max_size*: Maximum file size expected from the provider.
Default: `10MB`.
Values are can be expressed in human-readable form (e.g. `250KB` or `3MB`) and should be set per provider.
...
# Local file names will be determined by parsing URLs, but
# can be manually overridden:
filename_spam = spam_spam_spam_wonderful_spam.db
----
