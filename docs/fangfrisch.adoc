...
This will grant members of the `clamav` group the necessary write access for creating the database (see <<dbsetup>>).
...
After completing the configuration, make sure to create the database structure by running the `initdb` command as shown below in a root shell.
...
WARNING: As shown in this example, Fangfrisch should never be run as `root`, but as an unprivileged user like `clamav`.
...
*/15 * * * * clamav venv/bin/fangfrisch -c /etc/fangfrisch.conf refresh
----
