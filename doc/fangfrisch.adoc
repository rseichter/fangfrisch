...
Fangfrisch will process only enabled sections, processing lines with the prefix `url_`.
Supported schemas include FTP, HTTP and HTTPS.
The value of `integrity_check` determines both the expected filename for digests, e.g.
`http://example.tld/clamav/eggs.ndb.md5`, and the hashing mechanism used for verification.

NOTE: `max_age` is specified in minutes.
Providers usually document their own specific values.

NOTE: Set `integrity_check` to `disabled` if the provider does not support this type of checks.
